...
def findLanes(binary_warped):
    ...
    print('Histogram Method Curvature Values: Left Lane: {} m , Right Lane: {} m'.format(left_curverad, right_curverad))
...
def process_Image(img, mtx, dist):
    ...
    #Thresholds
    # kernel_size =5
    # undist_img = cv2.GaussianBlur(undist_img,(kernel_size, kernel_size),0)
    ...
    # lines_edges = cv2.addWeighted(warped, 0.8, line_image, 1, 0)
    ...
    return combined, undist_img, warped, Minv
...
output, undist_img, warped, Minv = process_Image(img, mtx, dist)
...
ploty, left_fitx, right_fitx = findLanes(warped)
